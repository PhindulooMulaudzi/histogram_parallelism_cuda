...
__global__ void histoGPU_2(uchar *input, uint *d_histo, long size){
	...
	// each threads stride by the number of threads in the grid. [Grid->Block]
	int stride = blockDim.x * gridDim.x;;
	
	for(int k = i; k < size; k+=stride)
	{
		int pos = input[k];
		if(pos >=0 && pos < 256)
			atomicAdd(&(d_histo[pos]), 1);
...
__global__ void histoGPU_3(uchar *input, uint *d_histo, long size, uint num_bins){
	...
	// create shared memory for local histogram and initialise element to zero
	...
	for(int k = i; k < size; k+=stride)
	{
		int pos = input[k];
		if(pos >=0 && pos < 256)
			atomicAdd(&(local_histo[pos]), 1);
	}
	__syncthreads();
	
	if (threadIdx.x < 256)
		atomicAdd(&d_histo[threadIdx.x], local_histo[threadIdx.x]);
}
...
int main(int argc, char **argv)
{
    ...
    for (int iter = 0; iter < numRuns; iter++){
		...
		histoGPU_3<<<grid_size, blk_size, BIN_NO * sizeof(uint)>>>(d_data, d_histo, byteCount, BIN_NO);
	}
	cudaDeviceSynchronize();
	sdkStopTimer(&hTimer);
